#!/bin/bash
#SBATCH --job-name=lingbot_quant_attn
...
#SBATCH --output=/home/%u/DLES/results/logs/%j_quant_attn.out
#SBATCH --error=/home/%u/DLES/results/logs/%j_quant_attn.err
...
cd ~/DLES/lingbot-va
QUANT_CONFIG=attention python -m torch.distributed.run \
    --nproc_per_node 1 \
    --master_port 29061 \
...
python -m evaluation.robotwin.eval_polict_client_openpi \
    ...
    --ckpt_setting quant_attn \
    --seed 0 \
    --policy_name LingBotVA \
    --save_root ~/DLES/results/quant_attn \
    --port 29056 \
    --test_num 16 \
